...
describe('Presenter', function() {
  ...
  it('should serialize in pure JS', function() {
    ...
    EventPresenter.prototype.attributes = () => ({ hej: 'test' })
  ...
  it('should use the sequelize adapter', function() {
    const { Presenter: PresenterSequelize } = yayson({ adapter: 'sequelize' })
    const obj = {
      get(attr) {
    ...
    const json = PresenterSequelize.toJSON(obj)
    return expect(json).to.deep.equal({
      data: {
        type: 'objects',
        id: '5',
  ...
  return it('can use custom adapters', function() {
    ...
    const PresenterWithMockAdapter = yayson({ adapter }).Presenter
